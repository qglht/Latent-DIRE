...
then
ssh $JNB_SSH_OPT sbatch --ntasks=1 --cpus-per-task=$JNB_NUM_CPU --time=${JNB_RUN_TIME}:00 --mem-per-cpu=$JNB_MEM_PER_CPU_CORE $JNB_SNUM_GPU --gres=gpumem:32g <<ENDSBATCH
#!/bin/bash
[ -n "$JNB_MODULE_USE" ] && module use "$JNB_MODULE_USE"
module load $JNB_MODULE_COMMAND
export XDG_RUNTIME_DIR=
